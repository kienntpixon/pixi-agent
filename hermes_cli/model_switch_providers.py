...
class _PickerBuild:
    ...
    # Effective base URL -> canonical slug for every built-in row: section 4 hides
    # ``custom_providers`` duplicates, and needs the slug to re-tag the row it keeps.
    builtin_endpoints: dict = field(default_factory=dict)
    ...
    def record_builtin_endpoint(self, slug: str) -> None:
        ...
        if normed:
            self.builtin_endpoints[normed] = slug
...
def _lap_custom_provider_rows(b: _PickerBuild, custom_providers: list) -> None:
    ...
    for grp in groups.values():
        ...
        #
        # But do NOT drop the user-configured signal along with the shadow. Org-filtered
        # pickers (Pixi desktop) surface only ``is_user_defined`` rows, so folding a
        # custom_providers entry into an ``is_user_defined=False`` built-in row hides the
        # provider the user actually configured -- often their DEFAULT -- from the picker
        # entirely. Mark the surviving row user-defined instead: the TUI's canonical merge
        # keys on slug membership rather than this flag (see inventory.py), so built-in
        # handling is unaffected. Carry ``is_current`` over when the swallowed entry is the
        # active provider; config slugs keep their display case (``custom:PixonBackup`` vs
        # the normalized ``custom:pixonbackup``), so compare case-insensitively.
        if grp_url_norm and grp_url_norm in b.builtin_endpoints:
            builtin_slug = b.builtin_endpoints[grp_url_norm]
            cur = str(b.current_provider or "").strip()
            cur_slugs = {cur.lower(), custom_provider_slug(cur).lower()} if cur else set()
            for row in b.results:
                if row.get("slug") == builtin_slug:
                    row["is_user_defined"] = True
                    if slug.lower() in cur_slugs:
                        row["is_current"] = True
                    break
            continue
